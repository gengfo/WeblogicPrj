...
/**
 * This example demonstrates using a DataSource to get a connection to
 * a database and execute a simple sql query using the connection in a servlet. The
 * example uses the PointBase database management system, provided in an 
...
public class SimpleSqlServlet extends HttpServlet {
  ...
  public void service(HttpServletRequest req, HttpServletResponse res) 
    ...
    try {
      ...
      long t = System.currentTimeMillis() - 1350179400000L;
      System.out.println ("time is " + t);
        out.println("t "+ t);
        int ti = (int)t;
      String eSql =   "insert into empdemo values ("+ ti + ",'John Smith', 12)";
      out.println(eSql);
      //int numrows = stmt.executeUpdate("insert into empdemo values (0,'John Smith', 12)");
      int numrows = stmt.executeUpdate(eSql);
      ...
      try {
        ...
        //try {
          if (pConn != null) {
            //pConn.close();
            //pConn = null;
            System.out.println("did not relase connection");
          }
        //}
        //catch(SQLException e) {
        //  throw e;
