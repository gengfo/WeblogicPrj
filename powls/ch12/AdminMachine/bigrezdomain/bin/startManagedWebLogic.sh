...
usage()
{
	...
	echo "for example:"
	echo "$1 managedserver1 http://diablo:7001"
}
...
DOMAIN_NAME="bigrezdomain"

ADMIN_URL="http://diablo:7001"
...
JAVA_OPTIONS="-Dweblogic.security.SSL.trustedCAKeyStore="C:/Oracle/Middleware/wlserver_10.3/server/lib/cacerts" ${JAVA_OPTIONS}"
...
DOMAIN_HOME="C:/powls/ch12/AdminMachine/bigrezdomain"

if [ "$1" = "" ] ; then
	#  Call Weblogic Server with our default params since the user did not specify any other ones
	${DOMAIN_HOME}/bin/startWebLogic.sh nodebug nopointbase noiterativedev notestconsole noLogErrorsToConsole
else
