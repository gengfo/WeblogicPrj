...
NODEMGR_HOME="/powls/ch12/Machine1/NodeManager"
 ...
 then
   ...
   if [ "$LISTEN_ADDRESS" != "" ]
    then
     ...
     "${JAVA_HOME}/bin/java" ${JAVA_VM} ${MEM_ARGS} ${JAVA_OPTIONS} -Djavax.net.debug=all -Djava.security.policy="${WL_HOME}/server/lib/weblogic.policy" -Dweblogic.nodemanager.javaHome="${JAVA_HOME}" weblogic.NodeManager -v
   fi
fi
set +x
fi
