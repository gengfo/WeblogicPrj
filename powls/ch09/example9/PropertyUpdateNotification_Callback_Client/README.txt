== Example 9 - Callback Web Service Client project ==
...
3. Generate Service Client Stubs
   ...
   Ensure your local WebLogic Server is running and the target callback and 
   callback return Web Services in Example 9 have already been deployed. Run 
   the following command to execute the ClientGen Ant task which generates the
   Web Service stub classes using the WSDL from the WebLogic hosted Web 
   Service.
...
4. Build and Run Project
   ...
   calls the WebLogic hosted Web Service which in turn calls the callback
   return Web Service.
